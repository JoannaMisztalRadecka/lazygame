...
//Camshift tracking from central point - works better with avoiding background but worse with mixing face
	void HandDetector::CamShiftTracking()
	{
		...
		int trackObject = 0;
		Point center, lastPosition;
		Point arrowPositionLeft, arrowPositionRight, arrowDown, arrowUp;
		arrowPositionLeft = Point(100,100);
		arrowPositionRight = Point(50,100);
		...
		{		
			...
			frame.copyTo(image);
			if(trackObject<0)
				circle( image, center , 10, Scalar(0,255,0), 5, 8, 0 );
		...
		for(;;)
		{
			...
			if( trackObject )
				{			
					...
					//drawing line
					if(center.x < lastPosition.x - 10)
					{
						arrowUp.x = arrowPositionLeft.x - 10;
						arrowUp.y = arrowPositionLeft.y + 10;
						arrowDown.x = arrowPositionLeft.x - 10;
						arrowDown.y = arrowPositionLeft.y - 10;

						line(image, arrowPositionLeft, arrowUp, cvScalar(0,255,0), 5);
						line(image, arrowPositionLeft, arrowDown, cvScalar(0,255,0), 5);
						
					}
					else if(center.x > lastPosition.x - 10) 
					{
						arrowUp.x = arrowPositionRight.x + 10;
						arrowUp.y = arrowPositionRight.y + 10;
						arrowDown.x = arrowPositionRight.x + 10;
						arrowDown.y = arrowPositionRight.y - 10;

						line(image, arrowPositionRight, arrowUp, cvScalar(0,255,0), 5);
						line(image, arrowPositionRight, arrowDown, cvScalar(0,255,0), 5);
						
					}
					
					line(image, arrowPositionLeft, arrowPositionRight, cvScalar(0,255,0), 5);
					lastPosition = center;
